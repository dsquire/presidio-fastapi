...
ERROR_COUNT = Counter(
    "http_errors_total", "Total count of HTTP errors", ["method", "endpoint", "status_code"]
)
ACTIVE_REQUESTS = Gauge(
    "http_active_requests", "Number of currently active HTTP requests", ["method", "endpoint"]
)
...
class PrometheusMiddleware:
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        monitored_path_suffixes = [
            suffix.strip()
            for suffix in settings.PROMETHEUS_MONITORED_PATHS.split(",")
            if suffix.strip()
        ]
...
def setup_prometheus(app: FastAPI) -> None:
    ...
    app.add_route(f"/api/{settings.API_VERSION}/metrics", metrics_endpoint())
